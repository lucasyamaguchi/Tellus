...
export const SkillsAndArtifactsView: React.FC = () => {
  ...
  const filteredSkills = skills.filter(s => {
    ...
    return s.agentAssigned === selectedAgentFilter || s.agentAssigned === 'all';
  });
      ...
      <div className="h-12 border-b border-card-border bg-sidebar px-4 flex items-center justify-between shrink-0 select-none">
        <div className="flex items-center space-x-2 bg-card rounded-lg p-0.5 border border-card-border text-xs">
          ...
          >
            ...
            <span>Artefatos & Entregas ({artifacts.length})</span>
          </button>
        </div>
      ...
      {activeTab === 'skills' && (
        <div className="flex-1 flex flex-col p-4 overflow-y-auto scrollbar-thin scrollbar-thumb-card-border space-y-4">
          ...
          <div className="flex items-center space-x-2">
            <span className="text-xs text-slate-400 font-semibold">Filtrar por Agente:</span>
            {['all', 'architect', 'debugger', 'fast_coder', 'custom'].map((ag) => (
              <button
                key={ag}
                ...
                className={`px-2.5 py-1 rounded-lg text-xs font-mono uppercase transition-all ${
                  selectedAgentFilter === ag
                    ? 'bg-brand-cyan/20 border border-brand-cyan/40 text-brand-cyan font-bold'
          ...
          {/* Create Skill Form Modal / Box */}
          {isCreatingSkill && (
            <form onSubmit={handleSaveNewSkill} className="p-4 rounded-2xl bg-card border border-accent/40 space-y-3 shadow-xl animate-in fade-in">
              <div className="flex items-center justify-between border-b border-card-border pb-2">
                <h4 className="font-bold text-xs text-slate-100 flex items-center space-x-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-accent-light" />
                  <span>Cadastrar Nova Skill para o Agente</span>
                </h4>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="ex: Next.js App Router Master"
                    className="w-full bg-background border border-card-border rounded-lg px-2.5 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-accent"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    placeholder="ex: Frontend, DevOps, SQL"
                    className="w-full bg-background border border-card-border rounded-lg px-2.5 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-accent"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-background border border-card-border rounded-lg p-2.5 text-xs text-slate-100 font-mono focus:outline-none focus:border-accent"
                />
              </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {filteredSkills.map((skill) => (
              <div
                key={skill.id}
                className="p-3.5 rounded-2xl bg-card border border-card-border hover:border-brand-cyan/40 transition-all flex flex-col justify-between space-y-2.5 group"
              >
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] font-mono uppercase px-2 py-0.5 rounded-full bg-accent/15 text-accent-light border border-accent/30">
                      {skill.agentAssigned || 'all'}
                    </span>
                  </div>
                ...
                <div className="p-2 rounded-xl bg-[#090b10] border border-card-border/60 text-[10px] font-mono text-slate-300 line-clamp-2">
                  {skill.promptInstructions}
                </div>

                <div className="flex items-center justify-between pt-1 border-t border-card-border/50 text-[10px] text-slate-500">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="w-72 border-r border-card-border bg-sidebar flex flex-col shrink-0">
            <div className="p-3 border-b border-card-border">
              <span className="text-[11px] uppercase font-bold text-slate-400 font-mono">
                Entregas do Projeto (.agentic/artifacts/)
              </span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1.5 scrollbar-thin scrollbar-thumb-card-border">
              {artifacts.length === 0 ? (
                <div className="p-6 text-center text-slate-500 text-xs">
                  Nenhum artefato gerado ainda. Peça ao agente para criar planos ou walkthroughs!
                </div>
              ) : (
                artifacts.map((art) => {
                  const isActive = selectedArtifact?.id === art.id;
                  return (
                    <div
                      key={art.id}
          ...
          {selectedArtifact ? (
            <div className="flex-1 flex flex-col bg-background overflow-hidden">
              ...
              <div className="flex-1 p-6 overflow-y-auto bg-[#090b10] scrollbar-thin scrollbar-thumb-card-border">
